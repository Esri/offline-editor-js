        ...
        return declare("O.esri.Edit.OfflineFeaturesManager", [Evented],
            {
                ...
                extend: function (layer, callback, dataStore) {
                    ...
                    // This is a fairly brittle solution to detecting if a feature layer collection
                    ...
                    // This approach leaves open the possibility that if the db is initialized
                    // and !layer.url then the library could encounter errors. At this time, it appears
                    // that the likelihood of this happening is fairly small.
                    if(!this._editStore._isDBInit) {
                        ...
                        //
                        //    // If we weren't able to find a url then we need to retrieve it from the feature layer
                        //    // dataStore.
                        //    if(success && !url) {
                        ...
                        //                deferred.resolve({success:success, message: message});
                        //            }
                        //            else {
                    ...
                    all(extendPromises).then(function (r) {
                        if(r.length === 0){
                            ...
                            //// Send event indicating that all the functions above were successfully
                            //// added to the feature layer.
                            //this.emit(this.events.EXTEND_COMPLETE, true);
                            //callback(r[0]);
                        }
                        else if(r[0].success){
                            callback(true, null);
