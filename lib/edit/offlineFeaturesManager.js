                ...
                _cleanDatabase: function(layer, tempId, addResults, updateResults, deleteResults) {
                    ...
                    if (updateResults.length > 0) {
                        if (updateResults[0].success) {
                            tempResult.layer = layer;
                            tempResult.id = updateResults[0].objectId;
                            edit = tempResult;
                        }
                    }
                    if (deleteResults.length > 0) {
                        if (deleteResults[0].success) {
                            tempResult.layer = layer;
                            tempResult.id = deleteResults[0].objectId;
                            edit = tempResult;
                        }
                    }
                    ...
                    this._editStore.delete(edit.layer, fakeGraphic, function (success, error) {
                        if (success) {
                            ...
                            this._editStore.deletePhantomGraphic(function(success,error){
                                if(!success) {
                                    console.log("_cleanDatabase delete phantom graphic error: " + error);
                                }
                                dfd.resolve({success: true, error: null, id: edit.id});
                            });
                        }
                        else {
                            dfd.reject({success: false, error: error, id: edit.id});
                        }
                    }.bind(this));
