        ...
        return declare("O.esri.Edit.OfflineFeaturesManager", [Evented],
            {
                    ...
                    layer.applyEdits = function (adds, updates, deletes, callback, errback) {
                        ...
                        all(promises).then(function (r) {
                                ...
                                featureCollections: [
                                    {
                                        ...
                                        featureLayerCollection: JSON.stringify(this.toJson())
                                    }
                                ]
                            };
                            ...
                            self._editStore._getFeatureCollections(function(success, result) {
                                if(success){
                                    var count = 0;
                                    result.featureCollections.forEach(function(entry, i){
                                        if(entry.featureLayerUrl === layer.url) {
                                            count++;
                                            result.featureCollections[i] = featureCollectionObject;
                                        }
                                    });

                                    if(count === 0) {
                                        result.featureCollections.push(featureCollectionObject);
                                    }
                                }
                                else if(!success && result === null) {
                                    result = featureCollectionObject;
                                }
                                else {
                ...
                */
                 ...
                 *
                 * It has the following signature: {id: featureLayerCollections: [{ featureCollection: [Object], featureLayerUrl: String }]}
                 * @param callback
                 */
