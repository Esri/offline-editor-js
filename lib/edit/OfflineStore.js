...
var OfflineStore = function(/* Map */ map) {
    ...
    this.getStore = function(){
        ...
        if(data != null){
            ...
            // jabadia: unclear, as property really gets array index values: 0,1,2,... not property names
            // suggestion, use split.forEach() instead
    ...
    this._addToLocalStore = function(/* Graphic */ graphic, /* FeatureLayer */ layer, /* String */ enumValue,callback){
        // jabadia: suggest to review the whole mechanism... too complex in my opinion, should be way simpler
        ...
        if(arr === null){
            setItem.success = this._setTempLocalStore(serializedGraphic);
    ...
    this._reestablishedInternet = function(){
        ...
        if(data != null){
            var dataArr = [];
            var split = data.split(this._localEnum().TOKEN);
            // jabadia: unclear, use .forEach() instead
            for(var property in split){
                var item = split[property];
    ...
    this._updateExistingLocalStore = function(/* String */ serializedGraphic){
        // jabadia: simplify code for this method... should work also for empty stores
        ...
        if(dupeFlag == false) {
            duplicateObj.success = this._setTempLocalStore(localStore + this._localEnum().TOKEN + serializedGraphic);
     ...
     */
     // jabadia: remove this, _updateExistingLocalStore() should suffice 
...
//                    var graphic = this._deserializeGraphic(test);
    ...
    this._serializeGraphic = function(/* Graphic */ graphic, layer, enumValue){
        ...
        return JSON.stringify(json); // jabadia: remove this and fix failing test cases
    }
