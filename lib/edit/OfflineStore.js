...
var OfflineStore = function(/* Map */ map) {
    ...
    this._applyEdits = function(/* Boolean */ internet, /* Graphic */ graphic,/* FeatureLayer */ layer, /* String */ enumValue, callback){
        ...
        if(internet === false){
            ...
            this._layerEditManager(null,graphic,layer,enumValue,this.enum(),function(count,success,id,error){
            ...
            case localEnum.DELETE:
                layer.applyEdits(null,null,[graphic],function(addResult,updateResult,deleteResult){
                    ...
                    function(error){
                        ...
                        mCallback(count,deleteResult[0].objectId,false,null,error);
                    }.bind(this)
                );
                break;
            case localEnum.ADD:
                layer.applyEdits([graphic],null,null,function(addResult,updateResult,deleteResult){
                    ...
                    function(error){
                        console.log("_layer: " + error.lineNumber + " " + error.message);
                        mCallback(count,addResult[0].objectId,false,null,error);
                    }.bind(this)
                );
                break;
            case localEnum.UPDATE:
                layer.applyEdits(null,[graphic],null,function(addResult,updateResult,deleteResult){
                    ...
                    function(error){
                        ...
                        mCallback(count,updateResult[0].objectId,false,null,error)
                    }.bind(this)
                );
                break;
        }
...
//                    var graphic = this._deserializeGraphic(test);
                    if(test == entry){
                        var setItem = "";
                        splitStore.splice(parseFloat(property),1);
//                        var test = this._reserializeGraphicsArray(splitStore);
//                        var firstIndex = localStore.indexOf(test);
//                        var lastIndex = localStore.lastIndexOf(test);
//
//                        localStorage.removeItem(this._localEnum().STORAGE_KEY);
//
...
//                        }
        ...
        if(setItem == true){
            this._dispatchEvent(true,this._localEnum().DELETE_ITEM_EVENT);
            callback(true)
            return true;
        }
        else{
